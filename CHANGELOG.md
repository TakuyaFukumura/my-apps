...
- ダークモード設定の読み込みによる hydration mismatch を修正

## [0.11.1] - 2026-08-30

### 変更

- テーマ切替ボタンの文字ラベルを削除し、アイコンのみの表示に変更
...
- アプリ名をクリックすると詳細画面（`/apps/:id`）へ遷移するリンクに変更
- カード全体をクリック可能にし、どこをクリックしても詳細画面へ遷移できるように変更
- 「サイトを見る」ボタンに `aria-label`（新しいタブで開く旨）を追加し、アクセシビリティを向上

### 変更

- ダークモード切り替えボタンのラベルを「切り替え後の状態」を示す文言に変更
    - ライトモード時: 「ライトモード」→「ダークモードにする」
    - ダークモード時: 「ダークモード」→「ライトモードにする」

### 削除

- 一覧画面の「詳細を見る →」リンクを削除（アプリ名リンク・カードクリックで代替）
- `docs/ui-ux-improvement.md`（実装完了のため削除）
